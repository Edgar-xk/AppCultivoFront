...
import { Espacio, EspacioN } from 'src/app/Interfaces/Espacio';
...
export class EspacioService {
        ...
        this.sectorService.GetSectorByID(id).subscribe(data1 => {
          ...
          if (data1.Espacios.length > 0) {
            if (data1.Espacios.length < (index + 1)) {
              data1.Espacios.push({ _id1: data._id, Nombre: data.Nombre });

            } else {
              data1.Espacios.splice(index, 1, { _id1: data._id, Nombre: data.Nombre });
            ...
            for (let i = 0; i < data1.NumEspacios ; i++) {
              data1.Espacios.push({ _id1: null, Nombre: "Modificar" });
            }
  ...
  public GetEspacioById(id){
    return this.http.get<Espacio>("http://localhost:4000/GetEspacioByID/"+id);
  }
